...
def check_clearence(drw, img, start_x, start_y, side_length):
    ...
    if black_pixel_ratio > 0:
        paint_hazard(drw, start_x, start_y)
...
def python_start(img, drw):
    ...
    check_clearence(drw, img, x_y_list[0], x_y_list[1], clearance_diameter)
    check_clearence(drw, img, x_y_list[2], x_y_list[3], clearance_diameter)
    ...
    short_gimp_filename = gimp_filename + "_P_" + '{0:.2f}'.format(x_y_list[4]) + "_" + '{0:.2f}'.format(
        x_y_list[5]) + "_D_" + '{0:.2f}'.format(x_y_list[6]) + "_" + '{0:.2f}'.format(x_y_list[7])
