...
def paint_hazard(drw,x,y):
	...
	pdb.gimp_context_set_brush(temp_brush)
...
def parse_x_y(line, x_y_list):
	# takes in a line and returns the float x,y coordinates found in it
	pos1 = line.find(' ')
	x_meters_string = line[:pos1]
	pos2 = line.find(' ', pos1 + 1)
	y_meters_string = line[pos1 + 1:pos2]
	x_y_list.append(float(x_meters_string))
	x_y_list.append(float(y_meters_string))
	return
...
# takes filename of .ply the ratio and the offset points returns the park and dock list "start and stop"
	...
	park_x_y_m.append(park_x_y[0])
	park_x_y_m.append(park_x_y[1])
	dock_x_y_m.append(dock_x_y[0])
	dock_x_y_m.append(dock_x_y[1])
	
	park_x_y[0] = int(round(park_x_y[0] * ratio)) + offset_list[0]
	park_x_y[1] = int(round(park_x_y[1] * ratio)) + offset_list[1]
	
	dock_x_y[0] = int(round(dock_x_y[0] * ratio)) + offset_list[0]
	dock_x_y[1] = int(round(dock_x_y[1] * ratio )) + offset_list[1]
	
	# get actual location in meters for the file name, either offset or ratio need to be casted to a float or else they return an int
	
	park_x_y_m[0] = park_x_y_m[0] + float(offset_list[0]) / ratio
	park_x_y_m[1] = park_x_y_m[1] + float(offset_list[1]) / ratio
	
	
	dock_x_y_m[0] = dock_x_y_m[0] + float(offset_list[0]) / ratio
	dock_x_y_m[1] = dock_x_y_m[1] + float(offset_list[1]) / ratio
	
	# build the list 
	start_and_stop_list.append(park_x_y[0])
	start_and_stop_list.append(park_x_y[1])
	start_and_stop_list.append(dock_x_y[0])
	start_and_stop_list.append(dock_x_y[1])
	
	
	start_and_stop_list.append(park_x_y_m[0])
	start_and_stop_list.append(park_x_y_m[1])
	start_and_stop_list.append(dock_x_y_m[0])
	start_and_stop_list.append(dock_x_y_m[1])
...
def parse_files(dir_path,ratio):
...
def python_start(img,drw):
	...
	x_y_list = parse_files(dir_path,m2p)
